package io.github.bfox1.SwordArtOnline.client.font;
...
public class CustomFont {
     ...
     */
    public CustomFont(Minecraft mc, ResourceLocation font, int size)
    {
     ...
     */
    public CustomFont(Minecraft mc, ResourceLocation font, int size, int startChar, int endChar)
    {
            ...
            try
            {
            	//File f = new File(CustomFont.class.getProtectionDomain().getCodeSource().getLocation().getPath());
            	InputStream stream = mc.getResourceManager().getResource(font).getInputStream();
            	//File file = new File((mc.getResourceManager().getResource(font);
            	g.setFont(Font.createFont(Font.TRUETYPE_FONT, stream).deriveFont((float)size));
                    /*if (font instanceof String)
                    {
                    	...
                    	g.setFont(Font.createFont(Font.TRUETYPE_FONT, new File())).deri
                    }
            ...
            texLoc = mc.getTextureManager().getDynamicTextureLocation(font.getResourceDomain(), new DynamicTexture(img));
    }
